...
TimeAnalyze time_preprocess("Preprocess Function", boolclk_preprocess);
...
int main(int argc, const char * argv[]) {
    ...
    // Path a video file
    string u_path_video = "/Users/Quang/Test_Input_Files/UD/testvideo640480.mp4";

    processVideo(u_path_video); //ProcessVideo
    
    time_preprocess.Print();
    time_video.Print();
...
void processVideo(string videoFilename) {
    ...
    while( capture.read(frame) ){
        ...
        if (debug){
            cv::imwrite("/Users/yakup/Test_output_Files/Debug/binary_warped.jpg", binary_warped);
